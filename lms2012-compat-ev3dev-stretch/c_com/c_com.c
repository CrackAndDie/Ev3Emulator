...
void      cComCloseFileHandle(FILE* pHandle)
{
...
void      cComCreateBeginDl(TXBUF* pTxBuf, UBYTE* pName)
{
	...
	if (-1 != FileHandle)
	{
		...
		if ((MAX_MSG_SIZE - Index) < pTxBuf->pFile->Size)
		{
			...
			ReadBytes = fread(&(pTxBuf->Buf[Index]), sizeof(UBYTE), (pTxBuf->BufSize - Index), pTxBuf->pFile->File);
		...
		else
		{
			...
			else
			{
				...
				ReadBytes = fread(&(pTxBuf->Buf[Index]), sizeof(UBYTE), pTxBuf->pFile->Size, pTxBuf->pFile->File);
...
void      cComCreateContinueDl(RXBUF* pRxBuf, TXBUF* pTxBuf)
{
	...
	{
		// Msg cannot hold complete file
		ReadBytes = fread(&(pTxBuf->Buf[SIZEOF_CONTINUEDL]), sizeof(UBYTE), (pTxBuf->BufSize - SIZEOF_CONTINUEDL), pTxBuf->pFile->File);
		pTxBuf->BlockLen = pTxBuf->BufSize;
	...
	else
	{
		...
		{
			// Complete msg can fit in buffer
			ReadBytes = fread(&(pTxBuf->Buf[SIZEOF_CONTINUEDL]), sizeof(UBYTE), (pTxBuf->pFile->Size - pTxBuf->pFile->Pointer), pTxBuf->pFile->File);
...
void      cComSystemCommand(RXBUF* pRxBuf, TXBUF* pTxBuf)
{
			...
			if (OK == cComCheckForSpace(ComInstance.Files[FileHandle].Name, pRxBuf->pFile->Size))
			{
				...
				while ((ComInstance.Files[FileHandle].Name[Tmp]) && (Tmp < (MAX_FILENAME_SIZE - 1)))
				{
					...
					if (Folder[Tmp] == '/')
					{
						...
						{
							if (mkdir_cst(Folder, DIRPERMISSIONS) == 0)
							{
				...
				if (pRxBuf->pFile->File >= 0)
				{
					...
					fwrite(&(pRxBuf->Buf[MsgHeaderSize]), sizeof(UBYTE), (size_t)BytesToWrite, pRxBuf->pFile->File);
	...
	case CONTINUE_DOWNLOAD:
	{
		...
		if ((FileHandle >= 0) && (ComInstance.Files[FileHandle].Name[0]))
		{
			...
			if (pRxBuf->pFile->File >= 0)
			{
				...
				fwrite((pContiDl->PayLoad), sizeof(UBYTE), (size_t)BytesToWrite, ComInstance.Files[FileHandle].File);
		...
		if (FileHandle >= 0)
		{
			...
			if (pTxBuf->pFile->File >= 0)
			{
				...
				if ((pTxBuf->MsgLen + SIZEOF_RPLYBEGINREAD) <= pTxBuf->BufSize)
				{
					...
					ReadBytes = fread(pReplyBeginRead->PayLoad, sizeof(UBYTE), pTxBuf->MsgLen, pTxBuf->pFile->File);
				...
				else
				{
					...
					ReadBytes = fread(pReplyBeginRead->PayLoad, sizeof(UBYTE), (pTxBuf->BufSize - SIZEOF_RPLYBEGINREAD), pTxBuf->pFile->File);
	...
	case CONTINUE_UPLOAD:
	{
		...
		if ((FileHandle >= 0) && (pTxBuf->pFile->Name[0]))
		{
			...
			if (pTxBuf->pFile->File >= 0)
			{
				...
				{
					// Read all requested bytes as they can fit into the buffer
					ReadBytes = fread(pReplyContinueRead->PayLoad, sizeof(UBYTE), (size_t)BytesToRead, pTxBuf->pFile->File);
					pTxBuf->BlockLen = BytesToRead + SIZEOF_RPLYCONTINUEREAD;
					pTxBuf->State = TXIDLE;
				}
				else
				{
					ReadBytes = fread(pReplyContinueRead->PayLoad, sizeof(UBYTE), (size_t)(pTxBuf->BufSize - SIZEOF_RPLYCONTINUEREAD), pTxBuf->pFile->File);
	...
	case BEGIN_GETFILE:
	{
		...
		if (FileHandle >= 0)
		{
			...
			if (pTxBuf->pFile->File >= 0)
			{
				...
				{
					// Read all requested bytes as they can fit into the buffer
					ReadBytes = fread(pReplyBeginGetFile->PayLoad, sizeof(UBYTE), pTxBuf->MsgLen, pTxBuf->pFile->File);
				...
				{
					// Read only up to full buffer size
					ReadBytes = read(pReplyBeginGetFile->PayLoad, sizeof(UBYTE), (pTxBuf->BufSize - SIZEOF_RPLYBEGINGETFILE), pTxBuf->pFile->File);
	...
	case CONTINUE_GETFILE:
	{
		...
		if ((FileHandle >= 0) && (pTxBuf->pFile->Name[0]))
		{
			...
			if (pTxBuf->pFile->File >= 0)
			{
				...
				{
					// Read all requested bytes as they can fit into the buffer
					ReadBytes = fread(pReplyContinueGetFile->PayLoad, sizeof(UBYTE), (size_t)BytesToRead, pTxBuf->pFile->File);
					pTxBuf->BlockLen = ReadBytes + SIZEOF_RPLYCONTINUEGETFILE;
					pTxBuf->State = TXIDLE;
				}
				else
				{
					ReadBytes = fread(pReplyContinueGetFile->PayLoad, sizeof(UBYTE), (size_t)(pTxBuf->BufSize - SIZEOF_RPLYCONTINUEGETFILE), pTxBuf->pFile->File);
					pTxBuf->BlockLen = ReadBytes + SIZEOF_RPLYCONTINUEGETFILE;
					pTxBuf->State = TXGETFILE;
				}
	...
	case ENTERFWUPDATE:
	{
		...
		if (USBDEV == ComInstance.ActiveComCh)
		{
			UpdateFile = fopen(UPDATE_DEVICE_NAME, "w+");

			if (UpdateFile >= 0)
			{
				fwrite(&Dummy, sizeof(UBYTE), 1, UpdateFile);
				fclose(UpdateFile);
				// system("reboot -d -f -i");
			}
		}
		...
		if (!pTxBuf->Writing)
		{
			...
			if (BytesRead)
			{
				...
				{ // in the middle of a write file command
					...
					fwrite(pRxBuf->Buf, sizeof(UBYTE), (size_t)BytesToWrite, pRxBuf->pFile->File);
					pRxBuf->pFile->Pointer += (ULONG)BytesToWrite;
					pRxBuf->RxBytes += (ULONG)BytesToWrite;
...
void      cComTxUpdate(UBYTE ChNo)
{
	...
	if (0 == pTxBuf->Writing)
	{
		...
		case TXGETFILE:
		{
			...
			if (MsgLeft > pTxBuf->BufSize)
			{
				ReadBytes = fread(pTxBuf->Buf, sizeof(UBYTE), (size_t)pTxBuf->BufSize, pTxBuf->pFile->File);
				pTxBuf->pFile->Pointer += ReadBytes;
				pTxBuf->SendBytes += ReadBytes;
				pTxBuf->State = TXGETFILE;
			}
			...
			{
				ReadBytes = fread(pTxBuf->Buf, sizeof(UBYTE), (size_t)MsgLeft, pTxBuf->pFile->File);
				pTxBuf->pFile->Pointer += ReadBytes;
				pTxBuf->SendBytes += ReadBytes;
